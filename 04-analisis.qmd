...
Regarding the main dependent and independent variables of this study, @fig-meanchange depicts their standardized average changes over the years. We observe an increase in the average level of market justice preferences in the most recent waves, which begins in 2019. The highest average appears for perceived economic inequality in 2016, although this variable shows a downward trend over time, with the exception of 2019, which can be associated with the “social crisis” in the country. Interestingly, while perceptions of economic inequality declined in the latest measurements (2022–2023), market justice preferences increased. The meritocracy measures remain stable, though the perception that individuals are rewarded for effort is slightly higher than the perception that they are rewarded for their intelligence
...
ggeffects::ggpredict(model = m9.1,
                     terms = c("merit_effort_mean",
                               "perc_inequality_mean [min, max]"),
                     ci_level = .95
                     ) %>% 
  plot()
...
predictions(m9.1,
            condition = c("merit_effort_mean", "perc_inequality_mean"),
            newdata = datagrid(merit_effort_mean = df_study1$merit_effort_mean,
                               perc_inequality_mean = seq(min(df_study1$perc_inequality_mean), max(df_study1$perc_inequality_mean), by= 0.1)),
            re.form = NA) %>% 
  ggplot(aes(y =estimate,x=merit_effort_mean, fill=perc_inequality_mean, color=perc_inequality_mean)) +
